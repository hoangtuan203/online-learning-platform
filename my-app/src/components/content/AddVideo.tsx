...
export const AddVideo: React.FC<AddVideoProps> = ({ onSubmit, errors }) => {
  ...
  const handleVideoFileChange = (e: any, fileType: string) => {
    const file = e.target.files[0];
    if (file) {
      setVideoForm(prev => ({ ...prev, [fileType]: file.name }));
    }
  };
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-indigo-500 transition cursor-pointer relative group">
            ...
            <div className="text-center pointer-events-none">
              ...
              <p className="text-xs text-gray-500 mt-1">{videoForm.thumbnail || 'Chọn ảnh'}</p>
            </div>
          </div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-indigo-500 transition cursor-pointer relative group">
            <input
              type="file"
              onChange={(e) => handleVideoFileChange(e, 'video')}
              className="absolute inset-0 opacity-0 cursor-pointer"
              accept="video/*"
            />
            <div className="text-center pointer-events-none">
              <Upload className="mx-auto mb-2 text-gray-400 group-hover:text-indigo-500 transition" size={32} />
              <p className="text-sm text-gray-700 font-medium">File Video</p>
              <p className="text-xs text-gray-500 mt-1">{videoForm.video || 'Chọn file'}</p>
            </div>
          </div>
        </div>
      </div>
