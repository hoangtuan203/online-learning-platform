...
import { ChevronDown, Play, FileText, HelpCircle, AlertCircle, Clock, BookOpen, CheckCircle } from "lucide-react";
import { CourseService } from "../../service/CourseService";
import { EnrollService } from "../../service/EnrollService";
import type { ContentResponse } from "../../service/CourseService";
import type { EnrollmentRequest, EnrollmentStatus } from "../../types/Enrollment";
...
const FALLBACK_IMAGE = "https://res.cloudinary.com/dm1alq68q/image/upload/v1761064155/avatar-user/l0srbfzbhbi4sxq94lid.jpg";
...
export default function CourseDetail() {
  ...
  const [courseData, setCourseData] = useState<ExtendedCourseDetail | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        // Fetch contents for curriculum, hours, lessons
        ...
        // Extend course with contents and calculated fields
        ...
        setError(err instanceof Error ? err.message : "Đã xảy ra lỗi khi tải chi tiết khóa học");
  ...
  const benefits = useMemo(() => {
    if (!courseData || !courseData.contents || courseData.contents.length === 0) {
      return [];
    }
    // Take first 5 contents and prefix with "Học về " to make it sound like benefits
    return courseData.contents.slice(0, 5).map((content) => `Học về ${content.title}`);
  }, [courseData]);

  // Build curriculum from contents (single module for simplicity)
  const buildCurriculum = (contents: ExtendedCourseDetail['contents']): CurriculumModule[] => {
    if (!contents || contents.length === 0) {
      return [];
    }

    // Sort contents by createdAt
    const sortedContents = contents.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  ...
  const priceDisplay = courseData.price ? `${courseData.price.toLocaleString('vi-VN')} VND` : "Miễn phí";
  ...
  return (
    <div className="bg-white min-h-screen">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4 text-sm text-gray-500">
                  <span className="font-medium">{hours} giờ</span> • {lessons} bài học
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        try {
                          ...
                          toast.success(response.message || 'Đăng ký khóa học thành công! Bạn có thể bắt đầu học ngay.');
                          setIsEnrolled(true);
                          navigate(`/course/learning/${courseData.id}`);
                        } catch (err) {
                          toast.error(err instanceof Error ? err.message : 'Đăng ký khóa học thất bại');
                      ...
                      disabled={enrolling}
                    >
                      {enrolling ? 'Đang đăng ký...' : `Đăng ký học ngay (${priceDisplay})`}
                    </button>
                    <Link
                      to={`/course/learning/${id}`}
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center md:text-left">Bạn sẽ học được gì?</h2>
        {benefits.length > 0 ? (
          ...
          <p className="text-center text-gray-500 py-8">Chưa có nội dung chi tiết để hiển thị lợi ích học tập.</p>
        )}
      </section>
          ...
          <div className="space-y-4">
            {curriculum.map((module, index) => (
              <div key={index} className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown className={`h-5 w-5 text-gray-500 transition-transform duration-300 ease-in-out ${openModules.includes(index) ? 'rotate-180' : ''}`} />
                </button>
                <div 
                  id={`module-${index}`}
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${openModules.includes(index) ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}
                >
                  <div className="border-t border-gray-100">
                    <ul className="divide-y divide-gray-100">
                      {module.items.map((item, i) => (
                        <li key={i} className="flex items-start gap-4 p-4 hover:bg-gray-50 transition-colors">
            ...
            {curriculum.length === 0 && (
              <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                <p className="text-gray-500">Chưa có nội dung cho khóa học này. Hãy kiểm tra lại sau!</p>
              </div>
            )}
          </div>
        </div>  
      ...
      {/* Instructor Section */}
      <section className="py-12 px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Giảng viên</h2>
      ...
      {/* Reviews Section */}
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Đánh giá từ học viên</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {DEFAULT_REVIEWS.map((review, index) => (
            <article key={index} className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Khóa học liên quan</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
